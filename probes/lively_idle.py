...
OUT = ROOT / 'artifacts/lively-idle'
...
class MotionHandler(Handler):
    ...
    def do_GET(self):
        if self.path == '/stage':
            ...
            content = (ROOT / 'artifacts/hop-before-driver.js').read_bytes()
        elif self.path == '/voice.wav':
            content = (ROOT / 'artifacts/voice-review/audition-diana-curious.wav').read_bytes()
        else:
            return super().do_GET()
...
async def record(browser, base, variant):
    ...
    await page.evaluate('''()=>{
      ...
      at(11000,()=>{state('listening');label.textContent='接到弹幕 · 探头';});
      at(12000,()=>{
      ...
      at(14000,()=>{send({type:'performance',expression:'happy',gesture:'bounce'});label.textContent='打趣 · 短促弹一下';});
      at(15200,()=>send({type:'performance',expression:'auto'}));
      at(19000,()=>{send({type:'performance',expression:'curious',gesture:'tilt'});label.textContent='疑惑 · 偏头停一下';});
      at(20500,()=>send({type:'performance',expression:'auto'}));
    ...
    subprocess.run([ffmpeg,'-y','-ss',str(start),'-i',str(OUT/f'{variant}.webm'),'-itsoffset',str(audio-start),'-i',str(ROOT/'artifacts/voice-review/audition-diana-curious.wav'),'-c:v','libx264','-threads','2','-crf','20','-pix_fmt','yuv420p','-c:a','aac','-t','26',str(OUT/f'{variant}.mp4')],check=True,capture_output=True)
