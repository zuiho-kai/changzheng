...
(() => {
  ...
  function performExpression(name) {
    ...
    const gesture = name === 'happy' ? 'shrug' : name === 'curious' ? 'peek' : name === 'surprised' ? 'bounce' : undefined;
    $('#live2d-frame')?.contentWindow?.postMessage({type:'performance',expression:name,gesture},location.origin);
    expressionTimer=setTimeout(()=>{avatarExpression='auto';performanceState(avatarStatus);},6500);
  }
  ...
  function onEvent(event) {
    ...
    switch (event.type) {
      ...
      case 'turn_finished':
        ...
        expressionTimer=setTimeout(()=>{avatarExpression='auto';performanceState(avatarStatus);},event.interrupted?0:800);
        if (presentation) {
