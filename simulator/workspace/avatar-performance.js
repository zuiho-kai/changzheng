...
(() => {
  ...
  class AvatarPerformance {
    ...
    bindGaze(){
      ...
      const eyes=[], meshes=draw.vertexPositions.map((vertices,index)=>({index,native:new Float32Array(vertices),rest:new Float32Array(vertices)}));
      ...
      this.core.update=()=>{
        ...
        for(const mesh of meshes){
          const vertices=draw.vertexPositions[mesh.index],rest=mesh.rest;
          for(let i=0;i<vertices.length;i+=2){
            const rx=(rest[i]/2+.5)*1254,ry=(rest[i+1]/2+.5)*1254;
            let dx=0,dy=0;
            if(mesh.index===0){
    ...
    gesture(name) {
      ...
      this.nextHop=now+2300+Math.random()*2300;
    }
    ...
    update(now, mouth=0) {
      ...
      if(!this.action && !this.attention && now>=this.nextHop && this.expression!=='serious' && this.state==='idle'){
      ...
      if(now>=this.nextPose && !this.attention && !this.action && this.state==='idle'){
        if(Math.random()>.22){
          const head=(Math.random()-.5)*(this.state==='speaking'?.45:.55);
          this.poseTarget={head,body:-head*.5+(Math.random()-.5)*.15,pitch:(Math.random()-.5)*.23};
        }
        this.nextPose=now+2400+Math.random()*2600;
      }
